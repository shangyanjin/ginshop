...
func SeedDB() {
	...
	if len(GetSetting("email")) == 0 {
		email := Setting{Code: "email", Title: "Contact email", Value: "email@example.com"}
		if err := db.Create(&email).Error; err != nil {
			panic(err)
		}
	}
	if len(GetSetting("order_email")) == 0 {
		email := Setting{Code: "order_email", Title: "Email for order notifications", Value: "order@example.com"}
		if err := db.Create(&email).Error; err != nil {
			panic(err)
		}
	}
	...
	if len(GetSetting("home_id")) == 0 {
		...
		home := Page{
			Title: "Welcome To goweb Skeleton Project",
			Description: `
	...
	if len(GetSetting("about_id")) == 0 {
		about := Page{Title: "About goweb", Description: "goweb is a skeleton project for basic e-shop web-sites. It demonstrates user authentication, 3 user roles: admin, manager & customer, product cart & checkout, email notifications, user dashboard for all roles, some ui animations via Animate On Scroll library, bootstrap 4 template, home page carousel and may be something more...", Published: true}
